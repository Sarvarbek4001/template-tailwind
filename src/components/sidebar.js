...
const navigation = [
  ...
  { name: "Team", to: "/about", icon: UsersIcon, current: false },
  { name: "Users", to: "/contact", icon: FolderIcon, current: false },
...
export default function Example() {
  ...
  const handleClick = (item) => {
    let data = navigations.map((i) => {
      if (i.name === item.name) {
        return { name: i.name, to: i.to, icon: i.icon, current: true };
      } else {
        return { name: i.name, to: i.to, icon: i.icon, current: false };
      }
    });
    setNavigations(data);
  };
